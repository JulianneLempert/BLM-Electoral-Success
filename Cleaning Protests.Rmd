...
may <- may %>%
  ...
  # Note that there are 3 cases where there is an EstimateLow and no EstimateHigh, no cases vice versa
  ...
  # Change to dichotomous variable
   mutate(across(
    c(Arrests, Participant_Injuries, Police_Injuries, Property_Damage, Tear_Gas),
    ~ case_when(
        # Any non-zero number or text describing estimates such as "several" or "many" corresponds to yes
      # checked source and gave estimates
      .x == "numerous" ~ "75",  
      .x == "several" ~ "25",
      .x == "many" ~ "10",
      .x == "more than 70" ~ "70",
...
august <- august %>%
  ...
  mutate(BLM = case_when(
    ...
    # Pro2.Anti1 == 0 denotes protest unrelated to Trump
...
september <- september %>%
  ...
  mutate(BLM = case_when(
    ...
    Pro.2..Anti.1. == 1 & str_detect(Claim, regex("black|police|racism|racial|
                                              confederate|brutality|oppression|supremacy")) ~ 1,
    # Pro.2..Anti.1. == 0 denotes protest unrelated to Trump
   # Detects character strings consistent with BLM protests in the unrelated to Trump category
...
for wru bisg states labeled check, use birdie bisg

117th congress for 2020 candidates
116th congress works for 2020 candidates no redistricting yet
