...
Specific dataset generously curated by the authors.
...
protests <- protests %>%
 ...
 mutate(arrests = ifelse(arrests_any == 0, "0", arrests),
  ...
  # Establish mean size for each unit of location
  group_by(state, locality) %>%
  mutate(local_mean = mean(size, na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(state) %>%
  mutate(state_mean = mean(size, na.rm = TRUE)) %>%
  ungroup() %>%
  # Assign the average size value of the jurisdiction to values missing size
  # If all values within a city or county are missing, assign the state mean
  mutate(size = ifelse(is.na(size), 
                       ifelse(is.nan(local_mean), state_mean, local_mean), 
                       size)) %>%
  # Variables no longer needed
  select(-local_mean, -state_mean)
```
