...
```{r, include=FALSE}
protests <- read.csv("cleaned_full_protests.csv")
protests$log_size <- log(protests$size)
```

```{r, echo=FALSE}
# Descriptive plot of protest size for full protest dataset
ggplot(protests, aes(x = log_size)) +
...
# Descriptive plot of participant injuries per state (that have 2020 Senate elections)
...
# Descriptive plot of protest frequency per congressional district
...
```{r, echo=FALSE}
# Set the following seed for reproducibility
set.seed(650)
...
# Logistic regression plot: Effect of Size on Black Democrats in House General Elections
...
# Logistic regression plot: Effect of Frequency on Black Republicans in House Primary Elections
```{r, echo=FALSE}
...
```{r, echo=FALSE}
# Logistic regression plot: Effect of Crowd Injuries on Black Candidates in City Elections
...
```{r}
# Predicted probabilities chart: Predicted Probability of a Black Democratic Winner in the State Legislature
...
# Predicted probabilities chart: Predicted Probability of a Black Republican Winner in the State Legislature
ggplot(marg_effects, aes(x = x, y = predicted)) +
...
# Predicted probability plot: Predicted Probability of a Black Winner in House General Elections
ggplot(marg_effects, aes(x = x, y = predicted)) +
