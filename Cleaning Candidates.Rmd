...
no results for black or white candidates
...
```{r}
house_protest_2020 %>%
  filter(black_success_rate == 1)
```
house_BISG %>% filter(race == "black" & party == "REPUBLICAN")
Black republican house candidates checked 
...
nothing for Black candidates or white candidates
...
note that this is just for general candidates

black candidates
- police injuries almost sig (+)
- property damage almost sig (+)
- chemical agents almost sig (+)

note that all of the above denote severity
...
Note that race is included in the locals dataset, so no BISG needed.
...
1 star for injuries police (-) for white candidates
almost sig for size (-) for white candidates
